...
class TagDao {
  ...
  Future<List<Map<String, dynamic>>> getTags(int idTask) async {
    ...
    return await db.rawQuery('''    
          SELECT tg.id_tag, tg.name FROM tags tg,tasks_tags tt 
          WHERE tt.id_task = $idTask AND tg.id_tag=tt.id_tag 
          GROUP BY tg.id_tag     
        ''');
  }

  Future<List<Map<String, dynamic>>> getTagsByIdTaskOrderName(int idTask) async {
    Database db = await instance.database;
    return await db.rawQuery('''    
          SELECT tg.id_tag, tg.name FROM tags tg, playlists_tags tt 
          WHERE tt.id_playlist = $idTask AND tg.id_tag=tt.id_tag 
          GROUP BY tg.id_tag    
          ORDER BY tg.name 
        ''');
  }
